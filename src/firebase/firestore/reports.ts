...
const getDateRange = (timeframe: 'daily' | 'weekly' | 'monthly', periods: number) => {
  const now = new Date();
...
export const fetchUserSessions = async (userId: string, startDate: Date, endDate: Date): Promise<FocusSession[]> => {
...
export const getDailyStats = async (userId: string, days: number = 7): Promise<DailyStats[]> => {
...
export const getWeeklyStats = async (userId: string, weeks: number = 4): Promise<WeeklyStats[]> => {
...
export const getMonthlyStats = async (userId: string, months: number = 6): Promise<MonthlyStats[]> => {
